...
public class WinInDom extends WinBase{
			...
			if(safeGuardCount==0) {
				...
				if(mode==0) {
					...
					if(dataPackage.stateA.equals("期末考")){
						...
						dataPackage.choiceA="go_to_SA";//点按钮4（待着按钮）返回stayup
                    ...
                    if(dataPackage.paperFinished == 0) 
						dataPackage.choiceA="readpaper";//点按钮1（自习按钮）返回selfstudy
					else //如果已经进入撰写论文阶段
						dataPackage.choiceA="writepaper";
				}
				else if(mode ==9){
					messagePanel.setVisible(false);
					dataPackage.trigSubEvent=false;
					...
					dataPackage.choiceA="DirectPublish";
				}
				else if(mode == 11) {
					dataPackage.choiceA="ShowTeacher";
				}
				/*		END OF YOUR CODE		*/
				//¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥要刷新事件这部分一定要加¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥
	...
	public WinInDom(EventManager mainGame,JFrame frame) {
		...
		JPanel PaperPublishPanel = new JPanel();	//4.论文发表
		PaperPublishPanel.setOpaque(false);
		PaperPublishPanel.setBounds(254, 129, 531, 363);
		backgroundPanel.add(PaperPublishPanel);
		PaperPublishPanel.setLayout(null);
		
		JPanel PaperPublishupperlevel = new JPanel();
		PaperPublishupperlevel.setOpaque(false);
		PaperPublishupperlevel.setBounds(0, 0, 531, 363);
		PaperPublishupperlevel.setLayout(null);
		
		JPanel PaperPublishbackground = new ImagePanel("imgsrc//对话框.png",0, 0, 531, 363);
		PaperPublishbackground.setOpaque(false);
		PaperPublishbackground.setBounds(0, 0, 531, 363);
		PaperPublishbackground.setLayout(null);
		
			JLabel PaperPublishlabel_1 = new JLabel("恭喜你，你的论文终于写完了~");
			PaperPublishlabel_1.setFont(new Font("Lucida Grande", Font.BOLD, 16));
			PaperPublishlabel_1.setBounds(95, 130, 285, 16);
		
			JLabel PaperPublishlabel_2 = new JLabel("下一步怎么做？");
			PaperPublishlabel_2.setFont(new Font("Lucida Grande", Font.BOLD, 16));
			PaperPublishlabel_2.setBounds(95, 173, 388, 16);
			
			JButton DirectPublishButton = new JButton();
			DirectPublishButton.setFont(new Font("Lucida Grande", Font.BOLD, 16));
			DirectPublishButton.setBorderPainted(true);
			DirectPublishButton.setBounds(130, 250, 120, 50);
			DirectPublishButton.setText("直接投稿");
			
			JButton ShowTeacherButton = new JButton();
			ShowTeacherButton.setFont(new Font("Lucida Grande", Font.BOLD, 16));
			ShowTeacherButton.setBorderPainted(true);
			ShowTeacherButton.setBounds(260, 250, 120, 50);
			ShowTeacherButton.setText("先给导师看看");
		
			PaperPublishupperlevel.add(PaperPublishlabel_1);
			PaperPublishupperlevel.add(PaperPublishlabel_2);
			PaperPublishupperlevel.add(DirectPublishButton);
			PaperPublishupperlevel.add(ShowTeacherButton);
			
		PaperPublishPanel.add(PaperPublishupperlevel);
		PaperPublishPanel.add(PaperPublishbackground);
		
		if(dataPackage.paperFinished < 100 || dataPackage.paperPublised == true) {
            PaperPublishPanel.setVisible(false);
		}
		paperPanel = new JPanel();
		paperPanel.setBounds(780, 240, 215, 300);
		paperPanel.setVisible(true);
		paperPanel.setOpaque(false);//注意要设成透明的
		paperPanel.setLayout(null);
			
			JLabel paperPanelLabel = new JLabel();
			paperPanelLabel.setForeground(Color.BLACK);
			paperPanelLabel.setBounds(80, 25, 70, 30);
			paperPanelLabel.setFont(new Font("新宋体", Font.BOLD, 16));
			paperPanelLabel.setText("论文进度");
			paperPanel.add(paperPanelLabel);
			
			JProgressBar Bar_paper = new JProgressBar();
			Bar_paper.setOrientation(SwingConstants.VERTICAL);
			Bar_paper.setForeground(Color.RED);
			Bar_paper.setBounds(99, 68, 22, 140);		
			
			JPanel paperPanelBackground =  new ImagePanel("imgsrc//WinResearch/paperprocess.png",0, 0,210, 200);
			paperPanelBackground.setOpaque(false);
			paperPanelBackground.setLayout(null);
			paperPanelBackground.setBounds(20, 40, 150, 220);
			paperPanel.add(paperPanelBackground);
			paperPanel.add(Bar_paper);
		
		backgroundPanel.add(paperPanel);
		if(dataPackage.paperFinished == 0) {
		...
		dialogPack.setLayout(null);
			...
			}else if(dataPackage.domEventFinish) {
				dialogContent.setText("接下来干什么呢？");//设置默认对话内容   zwj
			    dataPackage.domEventFinish = false;
			}
			else {
		...
		Bar_paper.setValue(dataPackage.paperFinished);
		/*********************************************			
		 * 【鼠标动作的设置】
		 ********************************************/
		...
		demoMouseListener clickDirectPublish = new demoMouseListener(10);
		demoMouseListener clickShowTeacher = new demoMouseListener(11);
		...
		clickDirectPublish.setButton(DirectPublishButton);
		clickShowTeacher.setButton(ShowTeacherButton);
		...
		DirectPublishButton.addMouseListener(clickDirectPublish);
		ShowTeacherButton.addMouseListener(clickShowTeacher);
		/*		END OF YOUR CODE		*/
