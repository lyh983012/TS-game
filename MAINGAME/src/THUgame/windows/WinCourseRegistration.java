...
public class WinCourseRegistration extends WinBase{
		...
		public void mouseReleased(MouseEvent e) {
			...
			case 4: //按钮4=“提交选课”
				...
				if(0 == courseCheckedCount) { //用户啥都没选就点了提交
					dataPackage.choiceA="MsgSDOK"; //直接重绘窗口 （这里可不可以改成直接return？这样不需要重画窗口
				}
				else {
			...
			case 6: //按钮6=“结束选课”
				...
				break;
			case 7: //按钮7=msgPanel里的“OK”
				if(dataPackage.stateA.equals("MsgSD")) {
					//这是提交/删除选课返回的信息
					dataPackage.choiceA="MsgSDOK";
				}
				if(dataPackage.stateA.equals("MsgFinish")) {
					//这是结束选课返回的信息
					dataPackage.choiceA="RandomResultFinish";
				}
				break;
			case 8: //按钮8=完成选课显示课表界面上的OKbutton
	...
	public WinCourseRegistration(EventManager mainGame,JFrame frame) {
		...
		JButton selectRCourseButton = new JButton("选择必修");
