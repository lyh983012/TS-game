...
public class WinResearch extends WinBase{
	...
	static private class demoMouseListener extends BaseMouseListener{
		...
		public int JudgeSelectValid(int i) {
			// 如果点击的区域是海域，则返回无效
			if (researchDataPackage.BasisMap[i] == 10) {
				return 0;
			}
			int i_valid = 0;
			// 判断点击区域周围的区域是否已经被点击
			if (i%7 != 0) {
				if (researchDataPackage.BasisMap[i-1] == 0)
					i_valid = 1;
			}
			if (i%7 != 6) {
				if (researchDataPackage.BasisMap[i+1] == 0)
			...
			if (i/7 != 0) {
				if (researchDataPackage.BasisMap[i-7] == 0)
					i_valid = 1;
			}
			if (i/7 != 4) {
				if (researchDataPackage.BasisMap[i+7] == 0)
		...
		public void mouseReleased(MouseEvent e) {
			...
			// 判断点击的方格是否有效，如果无效则返回
			else if(JudgeSelectValid(mode) != 1) {
				return;
			}
			
			// 把点击的方格变成绿色，表示该文献已经阅读
			else {
				researchDataPackage.BasisMap[mode] = 0;
				if (mode == 4 && researchDataPackage.i_map == 0)
					dataPackage.choiceA="Desertmap";
				else if (mode == 6 && researchDataPackage.i_map == 0) {
					dataPackage.choiceA="Forestmap";
				}
				else if (mode == 20 && researchDataPackage.i_map == 0) {
					dataPackage.choiceA="IceBornemap";
				}
				else if (mode == 34 && researchDataPackage.i_map == 0) {
					dataPackage.choiceA="Volcanomap";
				}
				else { 
					dataPackage.choiceA="RandomEffect";
					researchDataPackage.CurrentPaper = mode;
				}
			}
			}
		...
		//¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥这部分不允许改¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥
				...
				for (int i_y = 0; i_y < 5; i_y++) {
					for (int i_x = 0; i_x < 7; i_x++) {
						...
						if(researchDataPackage.i_map == 0)
							setFieldButtonIcon(researchDataPackage.BasisMap[i_x+7*i_y], FieldButtonlist[i_x+7*i_y]);
						else if(researchDataPackage.i_map == 1)
							setFieldButtonIcon(researchDataPackage.DesertMap[i_x+7*i_y], FieldButtonlist[i_x+7*i_y]);
						else if(researchDataPackage.i_map == 2)
							setFieldButtonIcon(researchDataPackage.ForestMap[i_x+7*i_y], FieldButtonlist[i_x+7*i_y]);
						else if(researchDataPackage.i_map == 3)
							setFieldButtonIcon(researchDataPackage.IceBorneMap[i_x+7*i_y], FieldButtonlist[i_x+7*i_y]);
						else if(researchDataPackage.i_map == 4)
							setFieldButtonIcon(researchDataPackage.VolcanoMap[i_x+7*i_y], FieldButtonlist[i_x+7*i_y]);
				...
				panel.setBounds(66, 120, 197, 290);
				...
				switch(researchDataPackage.i_map) {
				case 0:
