...
public class WinResearch extends WinBase{
	...
	static private class demoMouseListener extends BaseMouseListener{
		...
		public String JudgeSelectValid(int i, int i_map) {
			...
			case 3:
			case 4:
				if (dataPackage.characterEnergy<45) {
					dataPackage.notification="体力不足";
					return "体力不足";
				}
				break;
		...
		public void mouseReleased(MouseEvent e) {
			...
			// 判断点击的方格是否有效，如果无效则返回
			else if(!JudgeSelectValid(mode, dataPackage.researchDataPackage.i_map).equals("valid")) {
				dataPackage.notification = JudgeSelectValid(mode, dataPackage.researchDataPackage.i_map);
	...
	public WinResearch(EventManager mainGame, JFrame frame) {
		//¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥这部分不允许改¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥¥
				...
				noticePanelforThesis.setLayout(null);
					...
					ThesisConfirmLabel.setBounds(63, 80, 400, 120);
				...
				for (int i_y = 0; i_y < 5; i_y++) {
					for (int i_x = 0; i_x < 7; i_x++) {
						...
						if(dataPackage.researchDataPackage.justReadThesis) {
							FieldButtonlist[i_x+7*i_y].setVisible(false);
						}
					}
				}
